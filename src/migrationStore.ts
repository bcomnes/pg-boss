...
function getAll (schema: string): types.Migration[] {
  return [
    ...
    },
    {
      ...
      uninstall: [
        `ALTER TABLE ${schema}.queue DROP COLUMN priority_counts`,
        `ALTER TABLE ${schema}.queue DROP COLUMN track_priority_stats`,
        // Restore previous version of create_queue function (without track_priority_stats)
        `
        CREATE OR REPLACE FUNCTION ${schema}.create_queue(queue_name text, options jsonb)
        RETURNS VOID AS
        $$
    ...
    },
    {
      ...
      previous: 30,
      install: [
        `ALTER TABLE ${schema}.queue ADD COLUMN track_priority_stats bool NOT NULL default false`,
        `ALTER TABLE ${schema}.queue ADD COLUMN priority_counts jsonb`,
        `
        CREATE OR REPLACE FUNCTION ${schema}.create_queue(queue_name text, options jsonb)
        RETURNS VOID AS
        $$
