...
function getAll (schema: string): types.Migration[] {
  return [
    ...
    },
    {
      ...
      previous: 30,
      install: [
        `ALTER TABLE ${schema}.queue ADD COLUMN IF NOT EXISTS retry_jitter numeric NOT NULL DEFAULT 1`,
        `ALTER TABLE ${schema}.job ADD COLUMN IF NOT EXISTS retry_jitter numeric NOT NULL DEFAULT 1`,
        `
        CREATE OR REPLACE FUNCTION ${schema}.create_queue(queue_name text, options jsonb)
        RETURNS VOID AS
        $$
        ...
        BEGIN
          ...
          WITH q as (
            ...
            VALUES (
              ...
              COALESCE((options->>'retryBackoff')::bool, false),
              (options->>'retryDelayMax')::int,
              COALESCE((options->>'retryJitter')::numeric, 1),
              COALESCE((options->>'expireInSeconds')::int, 900),
              COALESCE((options->>'retentionSeconds')::int, 1209600),
              COALESCE((options->>'deleteAfterSeconds')::int, 604800),
              COALESCE((options->>'warningQueueSize')::int, 0),
      ...
      uninstall: [
        // Restore previous version of create_queue function (without retry_jitter)
        `
        CREATE OR REPLACE FUNCTION ${schema}.create_queue(queue_name text, options jsonb)
        RETURNS VOID AS
        $$
        ...
        LANGUAGE plpgsql;
        `,
        `ALTER TABLE ${schema}.job DROP COLUMN IF EXISTS retry_jitter`,
        `ALTER TABLE ${schema}.queue DROP COLUMN IF EXISTS retry_jitter`
      ]
    }
  ]
}
